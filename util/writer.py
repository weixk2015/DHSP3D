...
class Writer:
    ...
    def print_current_losses(self, epoch, i, losses, t, t_data):
        ...
        if self.opt.color:
            message = '(epoch: %d, steps: %d, avg_step_t: %.3f) loss: %.6f chamfer_loss: %.6f mse_loss: %.6f ' \
                      'lap_loss: %.6f consistent_loss: %.6f edge_loss: %.6f texture_loss: %.6f area_loss: %.6f' \
                      % (epoch, i, t / self.opt.print_freq, losses["loss"],losses["loss_chamfer"], losses["loss_mse"],
                         losses["loss_lap"],losses["loss_consistent"],losses["loss_edge"],losses["loss_texture"],losses["loss_area"])
        else :
            message = '(epoch: %d, steps: %d, avg_step_t: %.3f) loss: %.6f chamfer_loss: %.6f ' \
                      ' mse_loss: %.6f normal_loss: %.6f  edge_loss: %.6f color_loss: %.6f' \
                      % (epoch, i, t / self.opt.print_freq, losses["loss"],losses["loss_chamfer"],losses["loss_mse"],losses["loss_normal"],losses["loss_edge"],losses["loss_color"])
        print(message)
        with open(self.log_name, "a") as log_file:
            log_file.write('%s\n' % message)
