...
class Mesh:
    ...
    def __init__(self, file=None, opt=None, hold_history=False, export_folder=''):
        self.faces=self.vs = self.v_mask = self.filename = self.features = self.edge_areas = None
        self.edges = self.gemm_edges = self.sides = self.edge_points = None
        self.ve = self.edges_sides = self.v_normals = self.lap_idx = self.vc = self.uvs = \
            self.face_uvs = self.texture = self.text = None
        self.pool_count = 0
