...
def fill_mesh(mesh2fill, file: str, opt):
	...
	# else:
	mesh_data = from_scratch(opt, file=file)
	...
	mesh2fill.vc = mesh_data['vc']
	mesh2fill.uvs = mesh_data['uvs']
	mesh2fill.face_uvs = mesh_data['face_uvs']
	mesh2fill.texture = mesh_data['texture']
	mesh2fill.texture_np = mesh_data['texture_np']
	mesh2fill.text = mesh_data['text']
	...
	mesh2fill.edges_sides = mesh_data['edges_sides']
	mesh2fill.v_normals = mesh_data['v_normals']
	mesh2fill.lap_idx = mesh_data['lap_idx']
...
def from_scratch(opt, file=None):
	class MeshPrep:
		def __getitem__(self, item):
			return eval('self.' + item)

	mesh_data = MeshPrep()
	mesh_data.vs = mesh_data.edges = mesh_data.vc = None
	mesh_data.gemm_edges = mesh_data.sides = None
	mesh_data.edges_count = None
	mesh_data.ve = mesh_data.edges_sides = mesh_data.v_normals = None
	mesh_data.lap_idx = None
	mesh_data.v_mask = None
	mesh_data.filename = 'unknown'
	mesh_data.edge_lengths = None
	mesh_data.edge_areas = []
	mesh_data.vs, mesh_data.faces, mesh_data.vc, mesh_data.uvs, mesh_data.face_uvs, \
	mesh_data.texture, mesh_data.text ,mesh_data.texture_np= fill_from_file(mesh_data, file)
	mesh_data.v_mask = np.ones(len(mesh_data.vs), dtype=bool)
	mesh_data.faces, mesh_data.face_areas = remove_non_manifolds(mesh_data, mesh_data.faces)
	if opt.num_aug > 1:
		faces = augmentation(mesh_data, opt, mesh_data.faces)
	build_gemm(mesh_data, faces, mesh_data.face_areas)
	if opt.num_aug > 1:
		post_augmentation(mesh_data, opt)
	# mesh_data.features = extract_features(mesh_data)
	mesh_data.features = gen_random_features(mesh_data)  # change
	return mesh_data


#
def fill_from_file(mesh=None, file=None):
	if mesh is not None:
	...
	for line in f:
		...
		if not splitted_line:
			...
			if text is not None or len(uvs) > 0:
				face_uvs.append([int(c.split('/')[1]) - 1 for c in splitted_line[1:]])
			...
			uvs.append([float(v) for v in splitted_line[1:3]])
	f.close()
	vs = np.asarray(vs)
	if len(uvs) == 0:
		uvs = None
		...
		if "_part" in texture_name:
			texture_name = texture_name[:-6]
		texture_path = '%s.jpg' % texture_name
		if os.path.exists(texture_path):
			texture = get_image(texture_path)[1]
		texture_np_path = "%s.npy" % texture_path.split('.')[0]
		if os.path.exists(texture_np_path):
			texture_np = np.load(texture_np_path)
		face_uvs = np.asarray(face_uvs, dtype=int)
	if len(vc) == 0:
		vc = None
	else:
		vc = np.asarray(vc)

	if face_uvs is not None and len(face_uvs) >= 0:
		vc = np.zeros_like(vs)

		for i, face in enumerate(faces):
			face_uv = face_uvs[i]
			for j in range(3):
				if not vc[face[j]][0] == uvs[face_uv[j]][0]:
					vc[face[j]][2] += 1
				vc[face[j]][:2] = uvs[face_uv[j]]

		#vc[:,:2] = vc[:,:2] / vc[:,2:]
	faces = np.asarray(faces, dtype=int)
	assert np.logical_and(faces >= 0, faces < len(vs)).all()

	return vs, faces, vc, uvs, face_uvs, texture, text,texture_np
...
def build_gemm(mesh, faces, face_areas):
	...
	lap_idx = np.empty([len(mesh.vs), 24], dtype=np.int64)
	lap_idx.fill(-1)
	lap_count = np.zeros((len(mesh.vs)), dtype=np.int64)
	edges_sides = []  # indicating this edge's index of the vertex
	v_edge_count = np.zeros((len(mesh.vs)))  # number of which vertex's edges
	...
	for face_id, face in enumerate(faces):
		...
		for idx, edge in enumerate(faces_edges):
			...
			if edge not in edge2key:
				...
				edges_sides.append([v_edge_count[edge[0]], v_edge_count[edge[1]]])
				v_edge_count[edge[0]] += 1
				v_edge_count[edge[1]] += 1
				lap_idx[edge[0], lap_count[edge[0]]] = edge[1]
				lap_count[edge[0]] += 1
				lap_idx[edge[1], lap_count[edge[1]]] = edge[0]
				lap_count[edge[1]] += 1
	...
	mesh.edges_sides = np.array(edges_sides, dtype=np.int64)
	mesh.lap_idx = lap_idx
...
def compute_face_normals_and_areas(mesh, faces):
	...
	face_normals /= (face_areas[:, np.newaxis] + 1e-7)
	# assert (not np.any(face_areas[:, np.newaxis] == 0)), 'has zero area face: %s' % mesh.filename
	face_areas *= 0.5

	v_normals = np.zeros((mesh.vs.shape[0], 3), dtype=np.float32)
	face_count = np.zeros((mesh.vs.shape[0], 1), dtype=np.int64)
	for i in range(faces.shape[0]):
		v_normals[faces[i]] += face_normals[i]
		face_count[faces[i]] += 1
	v_normals = v_normals / face_count
	mesh.v_normals = v_normals / np.sqrt((v_normals ** 2).sum(axis=1))[:, np.newaxis]
...
def gen_random_features(mesh):
	edge_points = get_edge_points(mesh)
	set_edge_lengths(mesh, edge_points)
	with np.errstate(divide='raise'):
		try:
			features = np.random.uniform(0, 1, (6, mesh.edges_count))
			return features
		except Exception as e:
			print(e)
			raise ValueError(mesh.filename, 'bad features')
