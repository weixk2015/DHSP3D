...
from models.losses.loss import MeshPriorLoss, RGBPriorLoss
...
def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'lambda':
        ...
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.8, threshold=0.01,
                                                   patience=100, min_lr=0.00004,verbose=True)
    else:
...
def init_net(net, init_type, init_gain, gpu_ids):
    if len(gpu_ids) > 0:
        ...
        net.cuda(gpu_ids[0])
        net = net.cuda()
        net = torch.nn.DataParallel(net, gpu_ids) #DataParallel
    if init_type != 'none':
...
def define_classifier(input_nc, ncf, ninput_edges, nclasses, opt, gpu_ids, arch, init_type, init_gain):
    ...
    if arch == 'mconvnet':
        ...
        #up_convs = ncf[::-1] + [nclasses] #change
        up_convs = ncf[::-1] + [6] #output channel
        #up_convs = ncf[::-1] + [12] #output channel
...
def define_loss(opt):
    if opt.color:
        loss = RGBPriorLoss(opt)
    else:
        loss = MeshPriorLoss(opt)
    return loss
...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, unroll=0, residual=True,
        ...
        for _ in range(blocks):
            self.conv2.append(MeshConv(out_channels, out_channels))
            self.conv2 = nn.ModuleList(self.conv2)
        self.last_conv = MeshConv(out_channels, out_channels)
        if batch_norm:
            for _ in range(blocks + 1):
                self.bn.append(nn.InstanceNorm2d(out_channels))
            self.bn = nn.ModuleList(self.bn)
    ...
    def __call__(self, x, from_down=None , last_relu = True):
        return self.forward(x, from_down, last_relu)

    def forward(self, x, from_down , last_relu):
        from_up, meshes = x
...
class MeshDecoder(nn.Module):
    ...
    def forward(self, x, encoder_outs=None):
        ...
        fe = self.final_conv((fe, meshes),last_relu=False)
        return fe
